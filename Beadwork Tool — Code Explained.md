...
> **The freeze on big canvases was drawing, not memory (the real fix).** Measured on the desktop: repainting a 14,000-bead design took **~2.7 seconds** — *even with zero beads placed*, an empty grid took the same, because **filling thousands of detailed oval outlines is ~45× slower than filling plain rectangles**. That multi-second repaint (fired on every flood-fill, pan and zoom) is what "froze" iPad Safari — memory was never the problem (heap stayed ~10 MB). The fix is a stronger **level-of-detail rule** in `drawScene`: draw the pretty ovals **only when zoomed in close** (beads > ~6 px on screen *and* fewer than ~3,000 cells visible); otherwise fill each cell with a fast rectangle — which, sized to the cell, reads as a clean solid low-res image when you're zoomed out to see the whole piece. After this, a full **100×100 cm** design (~235,000 beads) flood-fills and recolours in **under a second** instead of freezing. Knobs: the `heavy` / `simple` thresholds near the top of `drawScene`.

> **Draw the beads, not the grid (the crash that killed a 100×100 canvas).** Even after the level-of-detail fix, an *empty* 100×100 cm canvas crashed Safari the moment you drew a curved line. Why: the bead-drawing loop walked **every cell in the grid** (a 100×100 cm canvas has ~313,000) and built a text key for each to ask "is there a bead here?" — so even with 5 beads placed, every repaint churned through 313,000 cells and threw away 313,000 short-lived strings. A curved stroke turns off the fast-stroke shortcut (a snapping quirk), so it did that **~60×/second** until memory blew up. The fix: **iterate the bead `Map` (the placed beads) and skip anything off-screen**, instead of scanning the whole grid. Now a 5-bead canvas draws 5 beads, not 313,000 empty cells — the renderer scales with *what you drew*, not *how big the canvas is*. (The empty-cell outline pass still walks cells, but only when zoomed in, where at most a couple thousand are visible.)
